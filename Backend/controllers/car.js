...
export const byLocPrice = async (req, res, next) => {
  try {
    const cars = await Car.find({'buyerId' : null,'location' : req.body.location , 'price' : {$lte: req.body.price}}).sort({price: -1});
    res.status(200).json(cars);
  } catch (error) {
    next(error);
  }
...
export const byname = async (req, res, next) => {
  try {
    const cars = await Car.find({'title' : req.query.title}).sort({price: -1});
    res.status(200).json(cars);
  } catch (error) {
    next(error);
  }
